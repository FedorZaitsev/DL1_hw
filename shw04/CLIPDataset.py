...
class CLIPDataset(Dataset):
    def __init__(self, image_path, image_filenames, captions, tokenizer):
        ...
        self.encoded_captions = self.tokenizer(self.captions, add_special_tokens=True, 
            truncation=self.truncation, max_length=self.max_tokenizer_length, padding=self.padding)
        # TODO: Encode caption using tokenizers.
    ...
    def __getitem__(self, idx: int) -> tp.Dict[str, tp.Union[torch.Tensor, str]]:
        ...
        import os
        image = Image.open(os.path.join(self.image_path, self.image_filenames[idx]))
        item['image'] = self.transforms(to_tensor(image))
